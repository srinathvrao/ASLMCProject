        ...
        for file in ["Srinath"]:
        	...
        	for vid in vids:
		        ...
		        while True:
		            ...
		            overlay_image, wrist = posenet.draw_skel_and_kp(
		                display_image, pose_scores, keypoint_scores, keypoint_coords,
		                min_pose_score=0.15, min_part_score=0.1)
		            cv2.imshow('posenet', overlay_image)
		            if frame_count in range(frame_length//2 -50,frame_length//2 +10):
			            if len(wrist) != 0:
				            ...
				            cv2.imshow('wrist_cropped_image', resiz)
				            # cv2.waitKey(0)
				            cv2.imwrite("letter_images/"+file+vid[0]+".png",resiz)
				            # break
